...
blog post, we'll take a quick look into the emerging trends at the conference by going through a
...
- [Model Architectures & Learning Methods](#model-architectures--learning-methods)
...
In behavioral analysis, the location and pose of agents is first extracted from each frame of a behavior video, and then the labels are generated for a given set of behaviors of interest on a frame-by-frame basis based on the pose and movements of the agents as depicted in the figure above.
...
To take this into consideration, the paper proposes MR-Net to explicitly model the multi-rater agreement or disagreement. 
...
This paper takes a contrarian approach, and proposes an operation called *involution* that switches the two properties of convolutions,
...
#### GeoSim: Realistic Video Simulation via Geometry-Aware Composition for Self-Driving ([paper](https://arxiv.org/abs/2101.06543))
...
then predicts the RGB value at the given coordinate as an output. Since the LIIF representation is continuous, we can query an arbitrary high target resolution up to x30 higher than the resolution encountered during training. 
...
the inputs and how they produce the predictions. In addition to leveraging the capability of a transformer layer to model the global context which important for sematic segmentation in order to obtain coherent masks.
...
DETR is first pre-trained on a pretext task designed specifically for object detection as a desired down-stream task, then fine-tunned for object detection.
...
to the decoder. For multi-patch detection, the patches are grouped into sets of patches and each set is assigned to a given object query, and the attention aggregation is applied with a masking operation so that each prediction is not dependent on the rest (see section 3.2).
...
#### DatasetGAN: Efficient Labeled Data Factory With Minimal Human Effort ([Paper](https://arxiv.org/abs/2104.06490))
...
#### Ranking Neural Checkpoints ([Paper](https://arxiv.org/abs/2011.11200))
...
#### Real-Time High-Resolution Background Matting ([Paper](https://arxiv.org/abs/2012.07810))
...
#### Im2Vec: Synthesizing Vector Graphics without Vector Supervision ([Paper](https://arxiv.org/abs/2102.02798))

Despite the large amount of generative methods for images, there a limited amount of approaches that operate directly on 
vector graphics and require direct supervison. To solve this, the paper proposes Im2Vec, a new neural network that generates complex vector graphics with varying topologies, and only requires indirect supervision from readily-available training imges with no vector counterparts.
