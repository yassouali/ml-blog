...
## The Different Flavors of Action Recognition

The most popular video understanding tasks is (human) action recognition, consisting of predicting human actions and behaviors for the entirety of the input video. While the objective seems straight forward, in the day to day life, activities of daily living vary greatly. For instance, the actions can be simple ones that are usually short in time, have distinct visual patterns and consist of a limited number atomic movements such as "backflip" or "cutting vegetables". To more elaborate actions that span a longer time horizon and consists of a series of simple actions, such as "cooking". Given such diversity of human actions and activities, the task of action recognition can take different forms and focus of different types of behaviors:


**- Trimmed and untrimmed action recognition:** In trimmed action recognition, the annotators have already removed all of the irrelevant parts of the video, and input only contains the parts depicting the target actions. As for untrimmed action recognition, the videos contain both irrelevant and relevant parts, which reduces the annotation cost and makes them more similar to real-word data.

<figure style="width: 70%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/trimmed-untrimmed.png' | absolute_url }}" alt="">
  <figcaption>
  Trimmed and untrimmed action recognition.
  </figcaption>
</figure>


**- Simple and complex action recognition:**
As discussed briefly above, a very natural way of categorizing the different type of action is into simple and complex actions ([Hussein et al.](https://arxiv.org/abs/1812.01289)). While simple action are short in time and are visually distinct, complex actions are usually composed of several simple one-actions, take much longer to unfold, and have large variations in their temporal duration and temporal order, making it more challenging but resulting in a richer understanding of video data.

<figure style="width: 50%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/complex-actions.png' | absolute_url }}" alt="">
  <figcaption>
  Examples of a complex task: "Cooking a Mean", where each instance (v1, v2 and v3) has different temporal orders and temporal spans. (Image source: <a href="https://arxiv.org/abs/1812.01289">Hussein et al</a>)
  </figcaption>
</figure>


**- Individual and group action recognition:**
Since the task of action recognition consists mostly of processing videos that are human-centric, in which the main subject of the scene is a human participating in the action. The type of actions in this context can be defined based on the role the actor(s) play ([Kong et al](https://arxiv.org/abs/1806.11230)). For instance, the task can consist of detecting: 1) individual actions with a single actor and simple daily actions, or 2) group actions with multiple actors, and 3) human interactions with two actors participating in the action or, 4) human-object interactions with a single actor interacting with the objects of the scene.

<figure style="width: 100%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/interaction-example.png' | absolute_url }}" alt="">
  <figcaption>
  Examples of different types of actions from a human-centric view. (Image source: <a href="https://arxiv.org/abs/1806.11230">Kong et al</a>)
  </figcaption>
</figure>


**- Coarse and fine-grained action recognition:**
The standard (coarse) action recognition datasets focus mostly on actions happening in visually distinct scenes, where object and background cues from only a few frames are sufficient to solve this classic coarse-grained action recognition task. As a results, the models learned focus mostly on contextual information irrelevant to the actions, making them less useful in real-world applications. To solve this, recent fine-grained action recognition datasets ([Shao el al.](https://arxiv.org/abs/2004.06704)) define a classification task which depends on more action-specific details and subtle scene differences in order to predict the correct action.

<figure style="width: 60%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/coarse-and-fine-actions.jpg' | absolute_url }}" alt="">
  <figcaption>
  Coarse and fine action recognition. (Image source: <a href="https://arxiv.org/abs/2104.09496">Zhang et al</a>)
  </figcaption>
</figure>

**- Short and long-term recognition:**
Instead of predicting the actions taking place in a short seconds long time span, which are based short-term patterns and local properties, the task of long-term action recognition aims at understanding the "full picture" of a long-form video ([Wu et al.](https://arxiv.org/abs/2106.11310)). Examples include understanding the storyline of a movie, the relationships among the characters, the message conveyed by their creators, the aesthetic styles, etc.

<figure style="width: 50%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/short-and-long-term.jpg' | absolute_url }}" alt="">
  <figcaption>
  Short and long-term action recognition. (Image source: <a href="https://arxiv.org/abs/2104.09496">Wu et al</a>)
  </figcaption>
</figure>


**- Third-person and first-person (egocentric) action recognition:**

While significant progress have been made using datasets and benchmarks that capture the actions from a third-person view, such models are not applicable in setting where the video stream in comming from a first-person or *egocentric* point of view, where the scene is observed through the eyes of actor actively engaged with its environment ([Grauman et  al.](https://arxiv.org/abs/2110.07058)). The task of egocentric action recognition consists of creating robust methods for recognize activities under such conditions of an egocentric point of view, where the video stream is for instance combing from devices robot agent, a VR/AR headset or a wearable cameras device. While the real world applications are numerous, egocentric data entails new challenges, such as requiring a 3D understanding of actor's physical surroundings and being robust to different input noise such camera shakes and blurred shots.

<figure style="width: 90%" class="align-center">
  <img src="{{ 'http://127.0.0.1:4000/ml-blog/images/VID-UN/egocentric.png' | absolute_url }}" alt="">
  <figcaption>
  Examples of egocentric actions. (Image source: <a href="https://ego4d-data.org/fig1.html">Ego4D</a>)
  </figcaption>
</figure>

**- Multi-modal and uni-modal recognition:**

In order to mimic how we, as humans, experience the world around us, where we see objects, hear sounds, read words,
feel textures and taste flavors, multi-modal action recognition consists of leveraging multiple modalities beyond 
a single visual modality, such as audio and language, for a more detailed understanding of the world.

Multi-modal video understanding has attracted increasing attention in recent years [55, 3, 129, 167, 154, 2, 105].
There are two main categories for multi-modal video understanding. The first group of approaches use multi-
...
Humans interact with their environment through these multiple sensory streams, combin-
